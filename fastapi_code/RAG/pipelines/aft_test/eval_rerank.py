...
TEST_DATASET = "RAG/datasets/brain/ZZ/test_data/test_result_final2_1.json"
...
def recall_at_k(test_data, k=5):
    ...
    for i, item in enumerate(test_data):   #粗排的结果
        ...
        rrf_topk = item["rerank_block_idx_global_cut"][:k]  #粗排返回来的结果
...
def mrr(test_data):
    ...
    for item in test_data:
        ...
        rrf = item["rerank_block_idx_global_cut"] 
        rank = None
...
def precision(test_data, k):   #topK 里有多少是对的 / K
    
    score = 0
    for blocks in test_data:
        
        #先去重
        find = set(blocks["rerank_block_idx_global_cut"][:k])
        true = set(blocks["reference_answer"])
        yes = len(true & find)
        score += yes / k
...
def evaluate(test_data):
    ...
    pre_score_1 = precision(test_data, 1)
    pre_score_3 = precision(test_data, 3)
    pre_score_5 = precision(test_data, 5)
    ...
    print(f"pre_score_1: {pre_score_1:.4f}")
    print(f"pre_score_3: {pre_score_3:.4f}")
    print(f"pre_score_5: {pre_score_5:.4f}")
